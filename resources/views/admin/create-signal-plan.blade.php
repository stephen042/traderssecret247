            ...
            <div class="main-content app-content mt-0">
                <div class="side-app">
                    ...
                    <div class="main-container container-fluid">
                        ...
                        <livewire:admin.create-signal-plan />
                        {{-- content here --}}
                                            ...
                                            <table class="table table-bordered text-nowrap border-bottom w-100 "
                                                ...
                                                <thead>
                                                    <tr>
                                                        ...
                                                        <th class="wd-15p border-bottom-0">Plan Signal Strength</th>
                                                        <th class="wd-15p border-bottom-0">Action</th>
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                    @foreach ($signal_plans as $counter => $plan)
                                                        <tr>
                                                            <td>{{ $counter + 1 }}</td>
                                                            <td>{{ $plan->name }}</td>
                                                            <td>${{ number_format($plan->amount, 2) }}</td>
                                                            <td>{{ $plan->duration }}</td>
                                                            <td>{{ $plan->signal_strength }}%</td>
                                                            <td>
                                                                <form action="{{ route('delete_signal_plan', [$plan->id]) }}"
                                                                    method="post">
                                                                    @method('DELETE')
                                                                    @csrf
